...
max_df = 0.3
min_df = 2  # choose desired value for min_df

# Read stopwords
with open('stops.txt', 'r') as f:
    stops = f.read().split('\n')
nlp = spacy.load('en')
stops.extend(nlp.Defaults.stop_words)

# Read data
print('reading text file...')
data_file = 'raw/ASHG-2019.v2.txt'
docs = []
with open(data_file, 'r') as f:
    for line in f:
        normalized_line = unicodedata.normalize("NFKD", line)
        line_components = normalized_line.split('\t')
        if len(line_components) >= 2:
            docs.append(' '.join(line_components[1:]))
...
### Roland's additional preprocessing
import re
vocab_aux = [w for w in vocab_aux if not re.search(r'^\d', w) and not re.search(r'\d{2}', w)]
###
...
path_save = './data/ashg_min_df_' + str(min_df) + '/'
